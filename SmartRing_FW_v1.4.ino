...
#define BAUDRATE 9600 //Baudrate for UART communication with VESC
...
//Read data and check communication status with VESC
  if (VescUartGetValue(measuredValues)) { //Succeed to communicate with VESC
    commVescSt = true;
    timerCommVesc = millis();
    resetTry = false;
    autoResetTry = false;
  }
  else if (millis() - timerCommVesc > commVescTimeout) { //Failed to communicate with VESC
    commVescSt = false;
  }
  ...
  if (commVescSt) { //VESC communication is alive
    ...
    if (initAnim) {
      for (uint8_t z=0; z<NUM_LEDS_RING;z++) {
        ring.setPixelColor(z+ringStartBit,0,0,0);
        ring.show();
        delay(100);
      }
      initAnim=false;
    ...
    instantSpeed = measuredValues.rpm/7.0*GEAR_RATIO*WHEEL_DIAM/5305.16; //exact formula : erpm/motorPolesPair/60*motorPulley/drivePulley*wheelDiam/1000*3.14159*3.6
        ...
        case 0 : //default menu : battery gauge
          ...
          if (lastPixel > 0) {
            for(uint8_t i=lastPixel;i>0;i--) {
              ring.setPixelColor(i-1+ringStartBit, 0,0,0);
            }
          }
          ...
          if ((micros()-timer2)>=1000) {
            ...
            ring.setPixelColor(lastPixel+ringStartBit, greenValFade, redValFade, blueValFade);
   ...
   if(!autoResetTry && (millis() - timerCommVesc) > (commVescTimeout + 5000)) {
    resetComm();
    autoResetTry = true;
   }
...
void rearLightOff() {
  for (uint8_t z=0; z<NUM_LEDS_REARLIGHT; z++){
      ring.setPixelColor(z+stickStartBit,0, 0, 0); //Set red medium brightness
  }
}

void rearLightNoBrake() { //Set 2 red dots at each end of the stick
  for (uint8_t z=0; z<NUM_LEDS_REARLIGHT; z++){
      ring.setPixelColor(z+stickStartBit,0, 0, 0);
  }
  for (uint8_t z=0; z<2; z++){
      ring.setPixelColor(z+stickStartBit,255, 0, 0); 
      ring.setPixelColor(NUM_LEDS_REARLIGHT-1-z+stickStartBit,255, 0, 0); 
  }
}

void rearLightBrake() {
  for (uint8_t z=0; z<NUM_LEDS_REARLIGHT; z++){
      ring.setPixelColor(z+stickStartBit,255, 0, 0); //Set red medium brightness
  }
}
...
void softReboot() {
  ...
  while(1)
  {
    //infinite loop for trigerring watchdog and reset
  }
}
